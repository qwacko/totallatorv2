<script lang="ts">
  ...
  $metadata.title = "Bills";

  const { paramsStore, resultStore } = pbAccounts.bills.subscribeList({
    initialQueryParams: {
      filter: { title: "" },
...
{#if $resultStore?.items}
  <table>
    {#each $resultStore.items as currentBill}
      <tr>
        <td>
          <button
            class="border border-gray-600 px-2 py-1 rounded-md bg-gray-200"
            on:click={() => pbAccounts.bills.delete(currentBill.id)}>x</button
          >
        </td>
