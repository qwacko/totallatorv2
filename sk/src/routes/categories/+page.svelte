...
{#if $resultStore?.items}
  <table>
    {#each $resultStore.items as currentCategory}
      <tr>
        <td>
          <button
            class="border border-gray-600 px-2 py-1 rounded-md bg-gray-200"
            on:click={() => pbAccounts.categories.delete(currentCategory.id)}
            >x</button
          >
        </td>
        <td>{currentCategory.group}</td>
        <td>{currentCategory.title}</td>
        <td>{currentCategory.combinedTitle}</td>
      </tr>
    {/each}
  </table>
